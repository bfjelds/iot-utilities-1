...
namespace emon
{
    class Program
    {
        ...
        {
            Console.WriteLine("EMon - EBOOT Monitor for Athens");
            if (args.Count() != 1)
            {
                Console.WriteLine("usage: emon <device name>");
                Console.WriteLine();
                Console.WriteLine("Watching all devices");
            }
            else
            {
                sWatcherName = args[0];
            ...
            BroadcastWatcher bWatcher = new BroadcastWatcher();
            bWatcher.OnPing += new BroadcastWatcher.PingHandler(BroadcastWatcher_OnPing);
            bWatcher.AddListeners();
            Console.WriteLine("Press <Enter> to exit");
            Console.ReadLine();
        }

        static void BroadcastWatcher_OnPing(string Name, string IP, string Mac)
        {
            if (string.Empty == sWatcherName || sWatcherName.ToLower() == Name.ToLower())
            {
                Console.WriteLine(string.Format("{0} {1} {2}", Name, IP, Mac));
            }
        }

        static string sWatcherName = string.Empty;
