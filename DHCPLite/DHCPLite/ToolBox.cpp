...
char* strncpyz(char* const psDestination, const char* const psSource, const unsigned int uiDestinationLength)
	...
	if(0 < uiDestinationLength)
		{
			int destLen= sizeof(psDestination) / sizeof(char);
		const size_t stSourceSength = strlen(psSource);
		strncpy_s(psDestination,destLen, psSource, min(stSourceSength+1, uiDestinationLength-1));
...
AnsiString::AnsiString(const char* const psAnsi, const char pcStringTerminator)
	...
	if(0 != psAnsi)
		...
		if(0 != stAnsiLength)
			...
			if(0 != m_psAnsi)
				{
					int destLen = sizeof(m_psAnsi) / sizeof(char);
					strncpy_s(m_psAnsi, destLen,psAnsi, stAnsiLength);
